...
function state_bounds(states_vec, gps; local_gps_flag=false, local_gps_data=nothing, local_gps_nns=nothing, metric=Euclidean(), delta_input_flag=false, approximate_σ_flag=false)
    ...
    Threads.@threads for (idx, state) in collect(enumerate(states_vec)) 
        ...
        if local_gps_flag
            ...
            image, σ_bounds = bound_image([state[:,1], state[:,end-1]], local_gps[tid].gps, theta_vec_train_sq_sub, theta_vec_all, image_prealloc, delta_input_flag=delta_input_flag, approximate_σ_flag=approximate_σ_flag) 
        else
            image, σ_bounds = bound_image([state[:,1], state[:,end-1]], post_gps, theta_vec_train_squared_all, theta_vec_all, image_prealloc, delta_input_flag=delta_input_flag, approximate_σ_flag=approximate_σ_flag)
        end
...
"""
function bound_image(extent, gps, theta_vec_train_squared, theta_vec, image_prealloc; delta_input_flag=false, data_deps=nothing, known_component=nothing, σ_ubs=nothing, approximate_σ_flag=false)
    # TODO: mod keyword handling and document
    ndims = length(gps) 
    image_extent = Vector{Vector{Float64}}(undef, ndims)
    σ_bounds = zeros(ndims) 
    for i=1:ndims 
        image_extent[i], σ_bounds[i] = bound_extent_dim(gps[i], extent[1], extent[2], theta_vec_train_squared[i], theta_vec[i], image_prealloc, approximate_σ_flag=approximate_σ_flag)
        if delta_input_flag
...
function bound_extent_dim(gp, lbf, ubf, theta_vec_train_squared, theta_vec, image_prealloc; approximate_μ_flag=false, approximate_σ_flag=true)
